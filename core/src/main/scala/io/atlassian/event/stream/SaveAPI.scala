  ...
  def apply[F[_]: LiftIO, KK, E, K, S](config: SaveAPI.Config[F], toStreamKey: K => KK, store: EventStorage[F, KK, S, E]): SaveAPI[F, KK, E, K, S] =
    new SaveAPI[F, KK, E, K, S] {
      override def batch(key: K, initialOps: NonEmptyList[Operation[S, E]])(implicit F: Monad[F], S: Sequence[S]): F[SaveResult[S]] = {
        ...
        def partialSaveResult[A](p: PartialResult[S, E]): SaveResult[S] =
          p.fold(_._1, identity)

        def unprocessedOps(p: PartialResult[S, E]): Option[NonEmptyList[Operation[S, E]]] =
          p.fold(a => Some(a._2), _ => None)

        def storeOperations(store: EventStorage[F, KK, S, E], key: KK, s: S, ops: NonEmptyList[Operation[S, E]], retryCount: Int): F[PartialResult[S, E]] = {
          ...
          val program =
            for {
              enumerated <- LiftIO[EitherT[F, UnprocessedResult[S, E], ?]].liftIO(Operation.enumerate(key, s, ops))
