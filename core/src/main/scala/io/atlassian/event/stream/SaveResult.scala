...
sealed trait SaveResult[K, S, V] {
  ...
  def fold[X](s: Snapshot[K, S, V] => X, r: NonEmptyList[Reason] => X, n: Snapshot[K, S, V] => X): X =
    this match {
      case Success(v) => s(v)
...
object SaveResult {
  ...
  case class Noop[K, S, V](value: Snapshot[K, S, V]) extends SaveResult[K, S, V]

  def success[K, S, V](a: Snapshot[K, S, V]): SaveResult[K, S, V] =
    Success(a)

  def noop[K, S, V](a: Snapshot[K, S, V]): SaveResult[K, S, V] =
    Noop(a)
